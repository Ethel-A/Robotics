...
def main(start, goal, cost_map):
    ...
    while not exit and not done:
        execute(start,goal,cost_map)
        msg = int(input('What do you want to do? 1. change map 2. change goal 3.exit'))
        if(msg == 1):
            global pause = True
            row = int(input('Enter row of changed cell: '))
            col = int(input('Enter column of changed cell: '))
            value = int(input('Enter new value of changed cell: '))
            #Some validation later
            set_value(col, row,value,cost_map)
            main(position,goal,cost_map)
        elif(msg == 2):
            global pause = True
            row = int(input('Enter row of changed cell: '))
            col = int(input('Enter column of changed cell: '))
            new_goal = (row,col)
            main(global position, new_goal, cost_map)
        else:
            global exit = True'''
